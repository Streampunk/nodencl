...
std::string GetDeviceType(cl_device_id id) {
  cl_device_type deviceType;
  clGetDeviceInfo(id, CL_DEVICE_TYPE, sizeof(cl_device_type), &deviceType, nullptr);
...
napi_value GetDeviceTypes(napi_env env, napi_callback_info info) {
  ...
  for ( uint32_t x = 0 ; x < deviceIds.size() ; x++ ) {
    napi_value element;
    std::string deviceName = GetDeviceType(deviceIds[x]);
    status = napi_create_string_utf8(env, deviceName.data(), deviceName.size(), &element);
    assert(status == napi_ok);
    status = napi_set_element(env, result, x, element);
    assert(status == napi_ok);
  }
